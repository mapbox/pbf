...
export function compileRaw(proto, options = {}) {
    ...
    }${writeContext(context, {...options, isTypeScript})}`;
}
...
function writeMessage(ctx, options) {
    ...
    if (isTypeScript) {
        code += writeTypeScriptInterface(ctx);
    }

    if (!options.noRead) {
        const readName = `read${ctx._name}`;
        code += `${writeFunctionExport(
            options,
            readName
        )}function ${readName}(pbf${isTypeScript ? ': Pbf' : ''}, end${isTypeScript ? '?: number' : ''}) {
    return pbf.readFields(${readName}Field, ${compileDest(
    ctx,
    isTypeScript
)}, end);
}
function ${readName}Field(tag${isTypeScript ? ': number' : ''}, obj${isTypeScript ? `: ${  ctx._name}` : ''}, pbf${isTypeScript ? ': Pbf' : ''}) {
`;
        for (let i = 0; i < fields.length; i++) {
            ...
            let fieldRead =
                type === 'map' ?
                    compileMapRead(readCode, name) :
                    repeated ?
                        packed ?
                            readCode :
                            `obj.${name}${optional}.push(${readCode})` :
                        `obj.${name} = ${readCode}`;

            if (oneof) {
                // @TODO: fix this
                fieldRead += `;${isTypeScript ? '\n//@ts-ignore' : ''} \nobj.${oneof} = ${JSON.stringify(name)}\n`;
            }
    ...
    if (!options.noWrite) {
        ...
        code += `${writeFunctionExport(
            options,
            writeName
        )}function ${writeName}(obj${isTypeScript ? `: ${ctx._name}` : ''
...
function compilePackedWrite(ctx, field) {
    ...
    case 'fixed64':
        ...
        break;
    case 'sfixed32':
        writeMethod = 'SFixed32';
        break;
    case 'sfixed64':
        writeMethod = 'SFixed64';
        break;
    case 'bool':
        writeMethod = 'Boolean';
        break;
    default:
        throw new Error(`Unexpected packed field type: ${fieldType}`);
    }

    return `pbf.writePacked${writeMethod}(${field.tag}, obj.${field.name})`;
}

function writeTypeScriptInterface(ctx) {
    const fields = ctx._proto.fields;
    let code = `export interface ${ctx._name} {\n`;

    for (const field of fields) {
        const typeScriptType = getTypeScriptType(ctx, field);
        const optional = !field.required ? '?' : '';
        const isArray = field.repeated || isPacked(field);
        const arraySymbol = isArray ? '[]' : '';
        code += `    ${field.name}${optional}: ${typeScriptType}${arraySymbol};\n`;
    }
...
function getTypeScriptType(ctx, field) {
    ...
    if (type) {
        if (type._proto.fields) return type._name;
        if (isEnum(type)) return type._name;
    }
    ...
    case 'bool':
        return 'boolean';
    case 'enum':
        return field.type.split('.').pop(); // Use the enum name
    case 'uint32':
    case 'uint64':
    case 'int32':
    case 'int64':
    ...
    case 'sfixed32':
    case 'sfixed64':
        return fieldShouldUseStringAsNumber(field) ? 'string' : 'number';
    case 'bytes':
        return 'Uint8Array';
    case 'map':
        return `{[key: string]: ${getTypeScriptType(ctx, {
            type: field.map.to,
        })}}`;
    default:
...
function getTypeAssertion(defaultValue, typeScriptType, isArray, isTypeScript) {
    if (!isTypeScript) return '';

    const arrayAssertion = isArray ? '[]' : '';

    if (defaultValue === undefined) {
        // Use double assertion for undefined values
        return ` as unknown ${typeScriptType}${arrayAssertion}`;
    }
...
function writeEnum(ctx, {legacy, isTypeScript}) {
    const enums = getEnumValues(ctx);
    ...
    code += `${legacy ? `const ${name} = exports.${name}` : `export const ${name}`
    } = ${JSON.stringify(enums, null, 4)};\n`;
...
function compileDest(ctx, isTypeScript) {
    ...
    for (const field of ctx._proto.fields) {
        ...
        const typeScriptType = isTypeScript ?
            `as ${getTypeScriptType(ctx, field)}${isArray ? '[]' : ''}` :
            '';
        const typeAssertion = isTypeScript ?
...
function compileFieldRead(ctx, field) {
    const type = getType(ctx, field);
    if (type) {
        if (type._proto.fields)
            return `read${type._name}(pbf, pbf.readVarint() + pbf.pos)`;
...
function compileFieldWrite(ctx, field, name) {
    ...
    if (type) {
        if (type._proto.fields)
            return `${prefix}Message(${field.tag}, write${type._name}, ${name})`;
...
function compileRepeatedWrite(ctx, field) {
    return `for (const item of obj.${field.name}) ${compileFieldWrite(
        ctx,
        field,
        'item'
    )}`;
...
function compileMapWrite(ctx, field) {
    ...
    return `for (const key of Object.keys(${name})) ${compileFieldWrite(
        ctx,
        field,
        `{key, value: ${name}[key]}`
    )}`;
...
function getDefaultValue(field, value) {
    ...
    let convertToStringIfNeeded = function (val) {
        return val;
    };
    if (fieldShouldUseStringAsNumber(field)) {
    ...
    case 'float':
    case 'double':
        return convertToStringIfNeeded(value ? parseFloat(value) : 0);
    case 'uint32':
    case 'uint64':
    case 'int32':
    case 'int64':
    ...
    case 'sfixed32':
    case 'sfixed64':
        return convertToStringIfNeeded(value ? parseInt(value, 10) : 0);
    case 'string':
        return value || '';
    case 'bool':
        return value === 'true';
